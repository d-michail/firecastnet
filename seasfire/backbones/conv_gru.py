...
class ConvGRUCell(nn.Module):
    ...
    def init_hidden(self, batch_size, input_size, device):
        ...
        return Variable(torch.zeros(batch_size, self.hidden_dim, height, width)).to(
            device
        )
...
class ConvGRU(nn.Module):
    ...
    def forward(
        ...
        if hidden_state is not None:
            ...
            hidden_state = self._init_hidden(
                batch_size=batch_size, input_size=input_size, device=input_tensor.device
            )
    ...
    def _init_hidden(self, batch_size, input_size, device):
        ...
        for i in range(self.num_layers):
            init_states.append(
                self.cell_list[i].init_hidden(batch_size, input_size, device)
            )
        return init_states
...
class ConvGRUSeg(nn.Module):
    def __init__(
        self, input_dim, hidden_dim, kernel_size, num_layers, num_classes, pad_value=0
    ):
        super(ConvGRUSeg, self).__init__()
